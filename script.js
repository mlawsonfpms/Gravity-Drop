...
const UI = {
    ...
    air1: document.getElementById('air-toggle-1'),
    ...
    air2: document.getElementById('air-toggle-2'),
...
let simState = {
    ...
    t1: 0, y1: 0, v1: 0, finished1: false,
    t2: 0, y2: 0, v2: 0, finished2: false,
};
...
function stepSimulation(panelNum, dt) {
    ...
    const isAirOn = UI[`air${panelNum}`].checked && UI[`location${panelNum}`].value === 'Earth';

    let t = simState[`t${panelNum}`];
    let y = simState[`y${panelNum}`];
    let v = simState[`v${panelNum}`];
    let finished = simState[`finished${panelNum}`];
    ...
    if (y >= DROP_DISTANCE_M) {
        y = DROP_DISTANCE_M;
        finished = true;
        UI[`time${panelNum}`].textContent = t.toFixed(2);
    }

    simState[`t${panelNum}`] = t;
    simState[`y${panelNum}`] = y;
    simState[`v${panelNum}`] = v;
    simState[`finished${panelNum}`] = finished;
...
function animate(timestamp) {
    ...
    if (!simState.finished1 || !simState.finished2) {
...
function handleStart() {
    ...
    simState.finished1 = simState.finished2 = false;

    UI.time1.textContent = '—';
    UI.time2.textContent = '—';
...
function handleReset() {
    ...
    simState = { isRunning: false, lastTime: 0, animationFrameId: null,
        t1: 0, y1: 0, v1: 0, finished1: false,
        t2: 0, y2: 0, v2: 0, finished2: false };
...
function init() {
    ...
    UI.item2.value = 'Piece of Paper (0.005 kg)';
